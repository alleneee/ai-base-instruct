"""
统一向量数据库API
根据客户端指定的数据库类型自动选择使用Milvus或Elasticsearch
"""
from typing import List, Dict, Any, Optional, Union
from fastapi import APIRouter, Depends, HTTPException, Query, Body, status
from pydantic import BaseModel, Field, conlist

from enterprise_kb.storage.datasource.base import DataSourceFactory
...
from enterprise_kb.storage.pool.milvus_pool import (
    get_milvus_pool, get_milvus_batch_inserter, get_milvus_batch_deleter, get_milvus_batch_updater
)
...
# 依赖项
async def get_datasource(db_type: DbType = Query(DbType.MILVUS, description="数据库类型")):
    """根据数据库类型获取相应的数据源"""
    if db_type == DbType.MILVUS:
    ...
    try:
        ...
        await datasource.disconnect()  # 连接将归还到连接池而不是关闭

async def get_connection_pool_stats(db_type: DbType):
    """获取连接池统计信息"""
    if db_type == DbType.MILVUS:
...
async def get_pool_statistics(db_type: DbType = Query(DbType.MILVUS, description="数据库类型")) -> Dict[str, Any]:
    """获取连接池统计信息"""
    try:
        stats = await get_connection_pool_stats(db_type)
        return {
            "success": True,
            "message": f"成功获取{db_type}连接池统计信息",
            "stats": stats
        }
...
async def get_collection_statistics(
    datasource = Depends(get_datasource)
) -> Dict[str, Any]:
    """获取向量集合统计信息"""
    try:
        # 获取集合信息
...
async def add_vector(
    ...
    try:
        # 获取维度要求
        required_dimension = datasource.config.dimension
        
        # 验证向量维度
        if len(data.vector) != required_dimension:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"向量维度不匹配，需要 {required_dimension} 维，提供了 {len(data.vector)} 维"
            )
...
async def batch_add_vectors(
    ...
    try:
        ...
        for item in data.vectors:
            if len(item.vector) != required_dimension:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"向量维度不匹配，需要 {required_dimension} 维，提供了 {len(item.vector)} 维"
                )
...
async def update_vector(
    ...
    try:
        ...
        # 验证输入向量维度
        if len(data.new_data.vector) != required_dimension:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"向量维度不匹配，需要 {required_dimension} 维，提供了 {len(data.new_data.vector)} 维"
            )
...
async def search_vectors(
    ...
    try:
        ...
        # 验证向量维度
        if len(query.vector) != required_dimension:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"查询向量维度不匹配，需要 {required_dimension} 维，提供了 {len(query.vector)} 维"
            )
...
async def clear_collection(
    db_type: DbType = Query(DbType.MILVUS, description="数据库类型"),
    datasource = Depends(get_datasource)
) -> Dict[str, Any]:
    """清空集合"""
    try:
        ...
        if success:
            # 根据数据库类型刷新对应的批处理器
            if db_type == DbType.MILVUS:
                await get_milvus_batch_inserter().async_flush()
                await get_milvus_batch_deleter().async_flush()
                await get_milvus_batch_updater().async_flush()
            elif db_type == DbType.ELASTICSEARCH:
            ...
            return {
                "success": True,
                "message": f"成功清空{db_type}向量集合"
            }
        else:
            return {
                "success": False,
                "message": f"清空{db_type}向量集合失败"
            }
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def configure_connection_pool(
    ...
    """配置连接池参数"""
    try:
        if config.db_type == DbType.MILVUS:
            pool = get_milvus_pool()
        elif config.db_type == DbType.ELASTICSEARCH:
            pool = get_elasticsearch_pool()
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"不支持的数据库类型: {config.db_type}"
            )
        ...
        return {
            "success": True,
            "message": f"成功更新{config.db_type}连接池配置",
            "details": {
...
async def flush_batch_processors(
    db_type: DbType = Query(DbType.MILVUS, description="数据库类型")
) -> Dict[str, Any]:
    """强制刷新指定数据库类型的所有批处理器队列"""
    try:
        details = {}
        ...
        return {
            "success": True,
            "message": f"成功刷新{db_type}所有批处理器队列",
            "details": details
        }
    except HTTPException:
        raise
...
async def check_health(
    ...
    try:
        ...
        if is_healthy:
            ...
            return {
                "success": True,
                "message": f"{db_type}向量数据库连接正常",
                "details": info
            }
        else:
            return {
                "success": False,
                "message": f"{db_type}向量数据库连接异常"
            }
    except Exception as e:
        return {
            "success": False,
